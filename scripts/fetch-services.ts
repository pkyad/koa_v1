...
const main = async (): Promise<void> => {
	...
	;(servicesToFetch as ServiceSpec[]).forEach(async (service) => {
		...
		const specPath = `${BASE_DIR}/${service.alias as string}Spec.json`
		...
		try {
			...
			const child = spawnSync(
				...
				[
					...
					`-i ${specPath} -o ${BASE_DIR}/${service.alias as string}`
				],
				{
					cwd: '.',
					encoding: 'utf-8',
...
`
			...
			fs.writeFileSync(
				`${BASE_DIR}/${service.alias as string}/index.ts`,
				indexTemplate
			)
			...
			fs.writeFileSync(`${BASE_DIR}/index.ts`, serviceIndexTemplate)
